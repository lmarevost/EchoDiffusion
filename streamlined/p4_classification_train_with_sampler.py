...
def get_transforms(input_size, train=True):
    ...
    if model_name == 'InceptionV3':
        ...
        transform = transforms.Compose([
            ...
            #transforms.Normalize(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225]) ])
        # if train:
        #     transform.transforms.append(transforms.RandomAffine(degrees=0, translate=(0.05, 0.05), scale=(0.95, 1.05)))
    
    return transform


class CustomImageDataset(datasets.ImageFolder):
    """Dataset class that builds its own sampler to handle class imbalance."""

    def get_class_frequencies(self):
        # Iterate over the dataset and count the number of samples in each class
        counter = Counter([label for _, label in self.samples])
        class_frequencies = [counter[i] for i in range(len(counter))]
        print("\nSampler info:")
        print(f'Class frequencies:')
        print(class_frequencies, 'Total:', sum(class_frequencies))
        return class_frequencies

    def get_sampler(self):
        # Calculate the weights for each class in the dataset
        class_weights = [1.0 / freq for freq in self.get_class_frequencies()]
        ...
        print('Length:', len(sample_weights))
...
def get_data_loaders(traindir, valdir, input_size, batch_size):
    ...
    '''
    # get transformer
    train_transform = get_transforms(input_size, train=True)
    transform = get_transforms(input_size)
    
    # Datasets from each folder
    data = {
        'train': CustomImageDataset(root=traindir, transform=train_transform),
        'val': datasets.ImageFolder(root=valdir, transform=transform),
    }

    # Dataloader iterators
    dataloaders = {
        'train': DataLoader(data['train'], batch_size=batch_size, sampler=data['train'].get_sampler()),  # sampler to better handle class imbalance
...
def train_model(model, dataloaders, criterion, optimizer, scheduler, num_epochs, is_inception=False):
    ...
    best_auprc = 0.0  # imbalanced data -> go for AUROC (maybe AUPRC?) instead of Accuracy
    ...
    for epoch in range(num_epochs):
        ...
        for phase in ['train', 'val']:
            ...
            for item in dataloaders[phase]:
                ...
                # updating class counter and index set
                if phase == 'train':
                    counter.update(labels.tolist())
                    idx_set.update(idx.tolist())
